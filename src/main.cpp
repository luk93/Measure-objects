...
int main()
{
	...
	createTrackbar("Odleglosc obiektywu od obiektu", window_name[0], &odleglosc, //len-messured item distance
			1000, NULL);
	createTrackbar("Dolna wartosc progowa 1", window_name[2], &lowerb, 255,	//down treshold of binary1
			NULL);
	createTrackbar("Gorna wartosc progowa 1", window_name[2], &upperb, 255,	//up treshold of binary1
			NULL);
	...
	while (waitKey(20) != 27)
	{										// capturing and analizing 27 fps
		capture >> frame;
		frame_counter++;
		if (frame_counter == 27)
		{					// count 27 frames and increment mt(messuring time)
			frame_counter = 0;
			mt_seconds++;
		}
		frame.copyTo(img);
		...
		cvtColor(img, hsv_img, CV_BGR2HSV);					//rbg to hsv conversion
		...
		findContours(cont, contours1, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0));	//find contours
		findContours(cont2, contours2, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0));	//find contours
		...
		if (i_cont1 >= 0)	//analise of binary1 (hsv split 1)
		{
			...
			rectangle(img, boundRect1.tl(), boundRect1.br(),					//drawing rectangle diagonals and flanks
					Scalar(125, 125, 125), 2, 8, 0);
			line(img, boundRect1.tl(), boundRect1.br(), Scalar(250, 250, 125), 1, 8, 0);
			line(img, Point(boundRect1.x + boundRect1.width, boundRect1.y),
					Point(boundRect1.x, boundRect1.y + boundRect1.height),
					Scalar(250, 250, 125), 1, 8, 0);
			...
			out << " Szerokosc: " << boundRect1.width << " Wysokosc: "
					<< boundRect1.height;
			szerokosc1 = boundRect1.width;													// width
			wysokosc1 = boundRect1.height;													// height
			s = out.str();
			putText(img, s, Point(10, 10), CV_FONT_HERSHEY_COMPLEX, 0.4,					//puting information about rectangles in img window
					Scalar(20, 40, 80), 1, 8);
		...
		if (i_cont2 >= 0)	//analise of binary2 (hsv split 2), same as 1st
		{
			...
			rectangle(img, boundRect2.tl(), boundRect2.br(),
					Scalar(125, 125, 125), 1, 8, 0);
			line(img, boundRect2.tl(), boundRect2.br(), Scalar(250, 250, 125),
					1, 8, 0);
			line(img, Point(boundRect2.x + boundRect2.width, boundRect2.y),
			...
			out1 << " Szerokosc: " << boundRect2.width << " Wysokosc: "
					<< boundRect2.height;
			szerokosc2 = boundRect2.width;
			wysokosc2 = boundRect2.height;
			st = out1.str();
			putText(img, st, Point(10, 40), CV_FONT_HERSHEY_COMPLEX, 0.4,
					Scalar(20, 40, 80), 1, 8);
		...
		st2 = out3.str();
		putText(img, st2, Point(450, 40), CV_FONT_HERSHEY_COMPLEX, 0.4,			//puting dimensions in mm in img window
				Scalar(20, 40, 80), 1, 8);
		stream_to_file << "Czas: " << mt_seconds << " s. Klatka: "				//puting dimensions in mm to stream_to_file
				<< frame_counter << " Obiekt 1: " << szerokosc1 << " x "
				<< wysokosc1 << " mm. Obiekt 2: " << szerokosc2 << " x "
				<< wysokosc2 << " mm\n";
